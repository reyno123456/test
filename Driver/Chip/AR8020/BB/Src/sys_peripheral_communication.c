...
void Baseband_Soft_Reset(void)
{
    uint8_t bData=0;
    uint8_t loop=0;
    printf("Baseband_Soft_Reset\n");
    
    BB_SPI_WriteByte(PAGE2, FSM_0, 0x82);
    BB_SPI_WriteByte(PAGE2, FSM_0, 0x80);
    //while(loop ++ < 1000);
    
    //bData = BB_SPI_ReadByte(PAGE_1, 0x00);
    //bData = BB_SPI_ReadByte(PAGE_1, 0x00);
    //BB_SPI_WriteByte(PAGE2, FSM_0, bData | 0x02);
    //while(loop ++ < 1000);
    //BB_SPI_WriteByte(PAGE2, FSM_0, bData & 0xFD);
    //printf("bData = %x \n", bData);
}
#if 0
/**
...
{
  if(SEL==SEL_SKY)  HAL_GPIO_WritePin(SEL_GS_GPIO_PORT, SEL_GS_PIN, GPIO_PIN_RESET);
  else HAL_GPIO_WritePin(SEL_GS_GPIO_PORT, SEL_GS_PIN, GPIO_PIN_SET);
}
...
//  uint8_t  data;
  ...
  if(tmp1 != RESET)
    {
     // __HAL_UART_CLEAR_FLAG(&Uart1Handle, UART_IT_RXNE);
     // data = Uart1Handle.Instance->DR;
    }
}
...
void USART3_IRQHandler(void)
{
    ...
    {
       //uart3_flag = 3;
      __HAL_UART_CLEAR_FLAG(&Uart3Handle, UART_IT_RXNE);
      //  c = (Uart3Handle.Instance->DR);
      //  printf("%c",c);
    }
}
