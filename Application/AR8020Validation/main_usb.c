...
int main(void)
{
  ...
  int *msoc_soft_rstn = (int *)0x40b00010;
  tmp = *msoc_soft_rstn;
  *msoc_soft_rstn = tmp & ~(1<<6);
...
static void StartThread(void const * argument)
{
  ...
  serial_puts("MSC_InitApplication done\n");
  
  /* Init Host Library */
  USBH_Init(&hUSBHost, USBH_UserProcess, 0);
  
  serial_puts("USBH_Init done\n");
  
  /* Add Supported Class */
  USBH_RegisterClass(&hUSBHost, USBH_MSC_CLASS);
  
  serial_puts("USBH_RegisterClass done \n");
  
  /* Start Host Process */
  USBH_Start(&hUSBHost);
        
  serial_puts("USBH_Start done \n");
  
  /* Register the file system object to the FatFs module */
  if(f_mount(&USBH_fatfs, "", 0) != FR_OK)
  {  
    //LCD_ErrLog("ERROR : Cannot Initialize FatFs! \n");
    serial_puts("ERROR : Cannot Initialize FatFs! \n");
  }
  
  for( ;; )
  {
    serial_puts("StartThread before messageget\n");
    event = osMessageGet(AppliEvent, osWaitForever);
    serial_puts("StartThread after messageget\n");

    if(event.status == osEventMessage)
    {
      serial_puts("StartThread before switch\n");
      print_str(event.value.v);
      serial_putc('\n');
